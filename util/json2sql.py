...
def _get_list_cols_nested_tables(table_name: str, json_data: dict):
    ...
    for key, value in json_data.items():
        if isinstance(value, (list, dict)):
            ...
            cols.append(key)
    _all_objects[table_name] = {"list_cols": cols, "nested_tables": nested_tables}


def _create_table(
    cols: list, table_name: str = "countries", reference_table: str = None
):
    _table = singular_dict.get(table_name, table_name.lower())
    _ref_table = (
        singular_dict.get(reference_table, reference_table.lower())
        if reference_table
        else ""
    )
    create_query = f"DROP TABLE IF EXISTS `{table_name}`;\nCREATE TABLE IF NOT EXISTS `{table_name}` (\n\t"
    if not _sqlite:
        create_query += f"`{_table}_id` bigint UNSIGNED PRIMARY KEY NOT NULL{',' if reference_table or cols else ''}"
        if reference_table:
        ...
        create_query += f"`{_table}_id` INTEGER PRIMARY KEY NOT NULL{',' if reference_table or cols else ''}"
        if reference_table:
...
def _insert_from_dict(
    json_data, nested_tables, reference_table, reference_table_id, table_name
):
    values = ""
    json_data.pop("id", None)
    current_pk = int(_all_objects[table_name]["current_pk"]) + 1
    values += (
        f"({current_pk}, {str(reference_table_id) + ', ' if reference_table else ''}"
    )
    _all_objects[table_name]["current_pk"] = str(current_pk)
    for key in json_data:
        if key not in nested_tables:
            value = (
                str(json_data.get(key)).replace("'", "\\'")
                if not _sqlite
                else repr(json_data.get(key))
            )
            values += f"'{value}', " if not _sqlite else f"{value}, "
    values = values.strip(", ") + ");\n\n"
    _all_objects[table_name]["values"] = (
        _all_objects[table_name]["values"].strip().replace(";", ",\n") + values
    )

    for table in nested_tables:
        get_sql(
            json_data=json_data.get(table),
            table_name=table,
...
def get_sql(
    ...
    if table_name not in _all_objects:
        ...
        _create_table(
            cols=_all_objects[table_name]["list_cols"],
            table_name=table_name,
            reference_table=reference_table,
        )
    cols, nested_tables = (
        _all_objects[table_name]["cols"],
        _all_objects[table_name]["nested_tables"],
    )
    _insert_into(
        json_data=json_data,
        table_name=table_name,
...
def json_2_sql(
    json_data: dict,
    sqlite: bool = False,
    database_name: str = "COUNTRIES",
    first_table_name: str = "countries",
):
